...
import { createThemeExport, densityMap, fontMap, gradientMap, palettes, radiusMap, sampleSelections, resolveAccentColor, shadowMap, typeScaleMap, validateSelections, type Selection } from './theme'
...
function EditorialPage() {
  return <div className="sample-page editorial-page">
    ...
    <div className="editorial-hero"><span className="preview-label accent-chip">ISSUE 04 / THE ART OF NOTICING</span><h3>Some places stay with us.</h3><p>On walking slowly, looking closer, and finding a new story in familiar streets.</p><div className="editorial-byline">WORDS BY THE MARGIN EDITORS <span>·</span> 6 MIN READ</div></div>
...
function Preview({ theme, index, page, onCopy, copied }: { theme: Selection; index: number; page: PageKind; onCopy: () => void; copied: boolean }) {
  ...
  const contrastAccent = resolveAccentColor(theme)
  const style = {
  ...
  return <article className={`theme-panel ${expanded ? 'is-expanded' : ''}`} aria-label={`Theme ${index+1} preview`}>
    <header className="panel-head"><div><span className="panel-number">0{index+1}</span><h2>Interpretation {index+1}</h2></div><div className="panel-actions"><button type="button" className="expand-preview" onClick={() => setExpanded(value => !value)} aria-pressed={expanded}>{expanded ? 'Restore width' : 'Expand width'}</button><button type="button" className="copy-theme" onClick={onCopy} aria-label={`Copy theme ${index+1} as JSON`}>{copied ? 'Copied' : 'Copy theme'} <Arrow diagonal /></button><span className="panel-dot" style={{ background: primary }} /></div></header>
...
export default function App() {
  ...
  return <main className="app-shell"><header className="app-header"><a className="app-logo" href="/" aria-label="Jev Theme Explorer home"><span className="logo-icon"><span/><span/><span/><span/></span><span>jev<span className="logo-slash">/</span>theme explorer</span></a><div className="header-right"><span className="header-mode"><span/> DESIGN WORKBENCH</span><span className="header-version">V1.0</span></div></header>
    <section className="workspace-intro"><div className="intro-copy"><h1>Explore three visual directions <em>instantly</em>.</h1></div></section>
    <section className="prompt-section" aria-label="Theme prompt"><div className="prompt-examples"><span>TRY A DIRECTION</span>{prompts.map(item => <button key={item} type="button" onClick={() => setPrompt(item.toLowerCase())}>{item} <span>↗</span></button>)}</div><form onSubmit={generate}><div className="prompt-row"><input id="prompt" aria-label="Your aesthetic" value={prompt} onChange={e => setPrompt(e.target.value)} maxLength={600} minLength={3} placeholder="Describe a visual direction…" required/><button disabled={loading || prompt.trim().length < 3} type="submit">{loading ? 'Generating…' : 'Generate themes'} <Arrow /></button></div></form>{error && <p className="error" role="alert">{error}</p>}</section>
    <section className="results" aria-label="Theme results"><div className="page-picker"><div><strong>PREVIEW PAGE</strong><span>Switch the content. Keep the three themes.</span></div><div className="page-tabs" role="tablist" aria-label="Preview page">{pages.map(item => <button key={item.id} type="button" role="tab" aria-selected={page === item.id} onClick={() => setPage(item.id)} title={item.description}>{item.label}</button>)}</div></div>{copyError && <p className="error" role="alert">{copyError}</p>}<div className={`preview-grid ${loading ? 'is-loading' : ''}`} aria-busy={loading}>{themes.map((theme,index) => <Preview key={index} theme={theme} index={index} page={page} onCopy={() => copyTheme(theme,index)} copied={copied === index} />)}</div></section>
